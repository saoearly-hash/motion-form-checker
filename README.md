...
- **オフライン対応**: 一度アクセスすればAIモデルを含めて端末に保存され、以降は電波・Wi-Fiがなくても使えます(体育館やジムなど電波が不安定な場所でも利用可能)
...
AIモデル(骨格検知ライブラリ本体・約15MB)は外部CDNには依存せず、`vendor/mediapipe/` にファイルとして同梱しています。さらに `sw.js`(Service Worker)がアプリ本体とこのAIモデルを端末に保存するため、**最初に一度オンラインでアクセスすれば、2回目以降はインターネット接続なしで動作します**(体育館やジムなど電波が不安定な場所を想定)。

- 初回アクセス時だけ、AIモデルのダウンロード(数MB〜十数MB)にネット接続が必要です
- 2回目以降は端末に保存された内容を使うので、機内モードでも起動・利用できます
- スマホ/iPadでは「ホーム画面に追加」しておくと、通常のアプリのように起動できて便利です

### コードを更新したときの注意(開発者向け)

オフラインキャッシュの仕組み上、コードを更新して再pushしても、**一度訪れた端末には古い版がキャッシュされたまま**になることがあります。見た目や動作を変更した場合は、[sw.js](sw.js)冒頭の `CACHE_VERSION` の値(例: `"v1"` → `"v2"`)を変更してからコミット・pushしてください。これにより、次回アクセス時に古いキャッシュが破棄され、新しい版がダウンロードされます。

## 注意点

- カメラは使用せず、動画ファイルのアップロードのみに対応しています。
- 精度を上げたい場合は `app.js` 内の `MODEL_URL` を lite → full / heavy モデルに変更できます(その分、初回ダウンロードと処理は重くなります)。full/heavyモデルを使う場合は、`vendor/mediapipe/` に該当する `.task` ファイルを追加でダウンロードして配置し直してください。
